...
from arviz.test.helpers import check_multiple_attrs
...
class TestDataPyMC3:
    ...
    def data(self, eight_schools_params, draws, chains):
        with pm.Model() as model:
            ...
            eta = pm.Normal("eta", mu=0, sd=1, shape=eight_schools_params["J"])
    ...
    def get_predictions_inference_data(
        ...
        with data.model:
            ...
            extended = pm.predictions_to_inference_data(
    ...
    def make_predictions_inference_data(
        ...
        with data.model:
            posterior_predictive = pm.sample_posterior_predictive(data.obj)
            idata = pm.predictions_to_inference_data(
                posterior_predictive,
    ...
    def test_ovewrite_model_coords_dims(self):
        ...
        with pm.Model(coords=coords):
            ...
            idata1 = pm.to_inference_data(trace)
            idata2 = pm.to_inference_data(trace, coords={"dim1": new_dim1}, dims={"beta": ["dim2"]})
    ...
    @pytest.mark.xfail(reason="DensityDist does no longer exist")
    ...
    def test_constant_data(self, use_context):
        ...
        with pm.Model():
            ...
            trace = pm.sample(100, tune=100)
            if use_context:
                inference_data = pm.to_inference_data(trace=trace)
    ...
    def test_no_trace(self):
        ...
        # Only prior
        inference_data = pm.to_inference_data(prior=prior, model=model)
        test_dict = {"prior": ["beta"], "prior_predictive": ["obs"]}
        fails = check_multiple_attrs(test_dict, inference_data)
        assert not fails
        # Only posterior_predictive
        inference_data = pm.to_inference_data(
    ...
    def test_priors_separation(self, use_context):
        ...
        if use_context:
            with model:
                inference_data = pm.to_inference_data(prior=prior)
        else:
            inference_data = pm.to_inference_data(prior=prior, model=model)
        fails = check_multiple_attrs(test_dict, inference_data)
        assert not fails
...
class TestPyMC3WarmupHandling:
    ...
    def test_save_warmup_issue_1208_after_3_9(self):
        with pm.Model():
            ...
            idata = pm.to_inference_data(trace, save_warmup=True)
            test_dict = {
                "posterior": ["u1", "n1"],
            ...
            with pytest.warns(UserWarning, match="Warmup samples"):
                idata = pm.to_inference_data(trace[-30:], save_warmup=True)
            test_dict = {
                "posterior": ["u1", "n1"],
                "sample_stats": ["~tune", "accept"],
                "~warmup_posterior": [],
